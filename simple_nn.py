...
def k_cross_validate_model(metadata, histone_data_object, y_test, batch_size, epochs, model_type, model_params, latent_size, gaussian_noise, df, k = 4):
    ...
    for train_index, val_index in kfold_data:
        ...
        model = create_nn(latent_size, model_params[0], model_params[1], 0.04, model_params[3])
...
def post_process(metadata, histone_data_object, histone_mark_str, y_test):
    ...
    df, val_metrics_array, min_train_loss_array, min_train_mse_array, min_train_mae_array, min_val_loss_array, min_val_mse_array, min_val_mae_array = k_cross_validate_model(metadata, histone_data_object, y_test, 16, 1000, "simple_nn 16 3 0.0003 0.0 0.01 50 0.2", [3, 0.0003, 0.0, 0.01], 50, 0.2, None)
