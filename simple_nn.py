...
def k_cross_validate_model(metadata, X_train, y_train, y_test, batch_size, epochs, model_type, k = 4):
    ...
    for i in range(k):
        ...
        age_predictions = np.squeeze(model.predict(validation_x))
        print(age_predictions)
        type_arr = np.full(validation_y.shape, model_type)

        if df is None:
            df_dict = {"Actual Age": validation_y, "Predicted Mean Age": prediction_distribution.mean().numpy().flatten(), "Predicted Std": prediction_distribution.stddev().numpy().flatten(), "Model Type" : type_arr}
