...
def test_model(X_train, X_test, y_train, y_test, histone_mark_str, data_transform = None, age_transform = None):
    ...
    if histone_mark_str == "H3K4me3":
        model = create_nn(300, 5, 0.0003, 0.0, 0.01) # Best Model: simple_nn 16 5 0.0003 0.0 0.01 300 0.1 / simple_nn 16 3 0.0001 0.1 0.01 450 0.2
        auto_encoder_args = [16, 300, 0.0, 0.01, 0.1, 0.0003]
    elif histone_mark_str == "H3K27ac":
        model = create_nn(150, 3, 0.0002, 0.05, 0.1) # Best Model: simple_nn 16 3 0.0002 0.05 0.1 150 0.2 / simple_nn 16 3 0.0003 0.0 0.1 50 0.2
        auto_encoder_args = [16, 150, 0.05, 0.1, 0.2, 0.0002]
    ...
    print(df)
    df.to_csv('/gpfs/data/rsingh47/masif/ChromAge/GEO-' + histone_mark_str + '_results.csv')
